...
// Parallelize implementation: save 1.5% commit time, 19% prover time, 23.5% verifier time
pub fn evaluation_tensor_product(eval_point: &[u128]) -> Vec<Vec<u16>> {
    let mut o = vec![int_to_bigbin(1)];

    for coord in eval_point {
        let int_bin = int_to_bigbin(*coord);

        // Explicitly specify the type of `o_times_coord`
        let o_times_coord: Vec<Vec<u16>> = o.par_iter().map(|x| big_mul(x, &int_bin)).collect();

        // Compute `new_o` by combining `o` and `o_times_coord` in parallel
        let new_o: Vec<Vec<u16>> = o
            .par_iter()
            .zip(o_times_coord.par_iter())
            .flat_map(|(x, y)| {
                let combined: Vec<u16> = x.iter().zip(y.iter()).map(|(&a, &b)| a ^ b).collect();
                let mut result = vec![combined];
                result.extend_from_slice(&[y.clone()]);
                result
            })
            .collect();
...
// parallelize implementation: save 33% prover time
pub fn computed_tprimes(
    rows_as_bits_transpose: &Vec<Vec<u8>>,
    row_combination: &Vec<Vec<u16>>,
) -> Vec<Vec<u16>> {
    ...
    // parallelize the computation of t_prime
    let t_prime = (0..k)
        .into_par_iter()
        .map(|j| {
            let mut local_t_prime = vec![0u16; m];
    ...
    // 结果转置以符合 t_prime 的结构
    let mut result = vec![vec![0u16; k]; m];
    for j in 0..k {
        for i in 0..m {
            result[i][j] = t_prime[j][i];
        }
    }
