...
use super::utils::{
    choose_row_length_and_count, pack_rows, extend_rows, 
    evaluation_tensor_product, transpose_bits, computed_tprimes,
    transpose, xor_along_axis};
use super::binary_field16::{uint16s_to_bits, big_mul, BinaryFieldElement16};
use super::merkle_tree::{merkelize, get_root};
...
pub fn prover(evaluations: &[u8], evalutaion_point: Vec<u128>) -> Proof {
    ...
    let (log_row_length, log_row_count, row_length, row_count) = choose_row_length_and_count(log_evaluation_count);
    ...
    let packed_columns = columns.iter()
    ...
    Proof {
        ...
        eval: computed_eval,
        t_prime,
        columns: challenges.iter().map(|&c| columns[c as usize].clone()).collect(),
        branches: challenges.iter().map(|c| get_branch(&merkle_tree, (*c).into())).collect(),
    }
}
...
mod tests {
    ...
    fn test_prover() {
        ...
        assert_eq!(result.root, vec![237, 216, 147, 34, 92, 247, 228, 153, 173, 169, 131, 177, 192, 61, 21, 78, 211, 52, 232, 196, 45, 237, 95, 175, 114, 196, 110, 120, 213, 131, 64, 60]);
        ...
        assert_eq!(result.branches[0][0], vec![93, 251, 171, 238, 223, 49, 139, 243, 60, 9, 39, 196, 61, 118, 48, 245, 27, 130, 243, 81, 116, 3, 1, 53, 79, 163, 215, 252, 81, 240, 19, 46]);
        assert_eq!(result.branches[0][1], vec![117, 231, 100, 194, 34, 254, 150, 92, 231, 217, 46, 180, 190, 247, 71, 93, 114, 98, 57, 172, 82, 119, 48, 229, 64, 230, 118, 239, 184, 26, 154, 48]);
    }
}